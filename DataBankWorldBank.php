...
class DataBankWorldBank{
    ...
    public function PickDatabaseList($selector,$att,$i,$did){
        ...
        $seriescount = 3;
        $timescount = 3;
        if($contriescount > 0 && $seriescount > 0 && $timescount){
            for ($c=0; $c < $contriescount; $c++) { 
                $this->pickCountry($c);
                // sleep(10);
    ...
    public function pickCountry($c){
    
        $parentDiv = $this->seleniumHandle->findElement('css', '.variableTable.availableView.table-dimension-C');
        $liElements = $this->seleniumHandle->findSubElementsWithParent($parentDiv, 'tag', 'li');
        $this->seleniumHandle->findAndClickElementWithParent($liElements[$c],'css','input[type="checkbox"][class="chkSelectElement"]');
      
        return true;
    }
    public function pickSeries($s){
        ...
        $liElements = $this->seleniumHandle->findSubElementsWithParent($parentDiv, 'tag', 'li');
        $item = $liElements[$s]->findElement(WebDriverBy::cssSelector('input[type="checkbox"][class="chkSelectElement"]'));
        $item->click();
        $this->clickDatabankSeriesButton();
    ...
    public function pickTime($t){
        $this->clickDatabankTimeButton();
        $parentDiv = $this->seleniumHandle->findElement('css', '.variableTable.availableView.table-dimension-T');
        $liElements = $this->seleniumHandle->findSubElementsWithParent($parentDiv, 'tag', 'li');
        $item = $liElements[$t]->findElement(WebDriverBy::cssSelector('input[type="checkbox"][class="chkSelectElement"]'));
        $item->click();
        $this->clickDatabankTimeButton();
